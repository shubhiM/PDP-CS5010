...
; q1.rkt: Implements the world with list of doodad stars and doodad squares
...
;    String Boolean DoodadType Integer Integer NonNegInt)
; INTERPRETATION:
...
; -- color is the color of the doodad after each core bounce which can any of
;     SquareColor/StarColor
...
; doodads-after-tick:  (Doodad -> Doodad) ListOfDoodads -> ListOfDoodads

; 
; doodads-star-after-tick : ListOfDoodads -> ListOfDoodads
; RETURNS: list of doodads of type star after the tick
; DESIGN STRATEGY: use HOF map on LOD

(define (doodads-star-after-tick doodads)
  ;; Doodad -> Doodad
  ;; RETURNS: A doodad-star like the given star with changed color
  (map (lambda (doodad) (doodad-star-after-the-tick doodad)) doodads))
...
(define (doodads-square-after-tick doodads)
  ;; Doodad -> Doodad
  ;; RETURNS: A doodad-square like the given square with changed color
 (map (lambda (doodad) (doodad-square-after-the-tick doodad)) doodads))
...
(define (doodads-star-after-key-event-c doodads)
  ;; Doodad -> Doodad
  ;; RETURNS: The given doodad-star after key-event c
  (map (lambda (doodad) (doodad-star-after-key-event-c doodad)) doodads))
...
(define (doodads-square-after-key-event-c doodads)
  ;; Doodad -> Doodad
  ;; RETURNS: The given doodad-square after key-event c
  (map (lambda (doodad) (doodad-square-after-key-event-c doodad)) doodads))
...
(define (place-doodads-square doodads scene)
   ;; Doodad Scene -> Scene
  ;; RETURNS: A scene with doodad-square placed in the scene
  (foldr
   (lambda
       (doodad rest-of-doodads)
     (place-doodad-square doodad rest-of-doodads))
   scene
   doodads))
...
(define (place-doodads-star doodads scene)
  ;; Doodad Scene -> Scene
  ;; RETURNS: A scene with doodad-star placed in the scene
  (foldr
   (lambda (doodad rest-of-doodads) (place-doodad-star doodad rest-of-doodads))
   scene
   doodads))
...
(define (doodads-after-age-increase-by-1 ds)
  ;; Doodad -> Doodad
  ;; RETURNS: The given doodad with age incremented by 1
  (map (lambda (doodad) (doodad-age-after-increase-by-1 doodad)) ds))
...
;; velocity vector constants for star and square doodads

(define STAR-DOODAD-VELOCITY-VECTOR-1 12)
(define STAR-DOODAD-VELOCITY-VECTOR-2 10)
(define SQUARE-DOODAD-VELOCITY-VECTOR-1 9)
(define SQUARE-DOODAD-VELOCITY-VECTOR-2 13)
...
(define (doodad-star-vx-after-key-event-t t)
  (cond
    [(= (remainder (+ t 1) 4 ) 1) (* -1 STAR-DOODAD-VELOCITY-VECTOR-1)]
    [(= (remainder (+ t 1) 4 ) 2) (* -1 STAR-DOODAD-VELOCITY-VECTOR-2)]
    [(= (remainder (+ t 1) 4 ) 3) STAR-DOODAD-VELOCITY-VECTOR-1]
    [(= (remainder (+ t 1) 4 ) 0) STAR-DOODAD-VELOCITY-VECTOR-2]))
...
(define (doodad-star-vy-after-key-event-t t)
  (cond
    [(= (remainder (+ t 1) 4 ) 1) STAR-DOODAD-VELOCITY-VECTOR-2]
    [(= (remainder (+ t 1) 4 ) 2) (* -1 STAR-DOODAD-VELOCITY-VECTOR-1)]
    [(= (remainder (+ t 1) 4 ) 3) (* -1 STAR-DOODAD-VELOCITY-VECTOR-2)]
    [(= (remainder (+ t 1) 4 ) 0) STAR-DOODAD-VELOCITY-VECTOR-1]))
...
(define (doodad-square-vx-after-key-event-q q)
  (cond
    [(= (remainder (+ q 1) 4 ) 1) SQUARE-DOODAD-VELOCITY-VECTOR-1]
    [(= (remainder (+ q 1) 4 ) 2) SQUARE-DOODAD-VELOCITY-VECTOR-2]
    [(= (remainder (+ q 1) 4 ) 3) (* -1 SQUARE-DOODAD-VELOCITY-VECTOR-1)]
    [(= (remainder (+ q 1) 4 ) 0) (* -1 SQUARE-DOODAD-VELOCITY-VECTOR-2)]))
...
(define (doodad-square-vy-after-key-event-q q)
  (cond
    [(= (remainder (+ q 1) 4 ) 1) (* -1 SQUARE-DOODAD-VELOCITY-VECTOR-2)]
    [(= (remainder (+ q 1) 4 ) 2) SQUARE-DOODAD-VELOCITY-VECTOR-1]
    [(= (remainder (+ q 1) 4 ) 3) SQUARE-DOODAD-VELOCITY-VECTOR-2]
    [(= (remainder (+ q 1) 4 ) 0) (* -1 SQUARE-DOODAD-VELOCITY-VECTOR-1)]))
...
; DESIGN STRATEGY: use HOF filter on LOD

(define (doodads-after-key-event-dot ds)
  ;; Doodad -> Boolean
  ;; RETURNS: true iff the given doodad is of same age as the first doodad
  ;; in the list of doodads
  (reverse
   (filter (lambda (d) (not (doodad-of-the-same-age? (first ds) d))) ds)))
...
(define (doodads-after-mouse-event doodads mx my mev)
  ;; Doodad -> Doodad
  ;; RETURNS: The given doodad after applying the mouse event to it
  (map (lambda (doodad) (doodad-after-mouse-event doodad mx my mev)) doodads))
