...
; 170410 / wdc Upgraded to R7RS, sizes doubled for comparison with 64-bit Java
;
...
(let* ((m 200)
       ...
       (k 10)
       (l 1)
       (argc (- (length cmdline) 1))
       (m (if (< argc 1) m (string->number (list-ref cmdline 1))))
       (n (if (< argc 2) n (string->number (list-ref cmdline 2))))
       (k (if (< argc 3) k (string->number (list-ref cmdline 3))))
       (l (if (< argc 4) l (string->number (list-ref cmdline 4)))))
  (if (and (number? m)
           (number? n)
           (number? k)
           (number? l))
